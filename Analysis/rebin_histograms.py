...
def rebin_histograms(n_of_tagged_jets=0):
    QCDFile = ROOT.TFile.Open("FittingArea/QCD_Run2_0_b-tag.root".format(n_of_tagged_jets))
    TTbarFile = ROOT.TFile.Open("FittingArea/TTbar_Run2_0_b-tag.root".format(n_of_tagged_jets))
    
   
    SignalFile = ROOT.TFile.Open("FittingArea/XToHY_6b_2000_1100_pass_Run2.root","UPDATE")
    ...
    for region in ['signal', 'control', 'control_alternative']:
        ...
        SignalFile.WriteObject(dummy_unwrapped_signal, "unwrapped_dummy_{0}".format(region))
